...
모델링으로 들어가기 전에 짚어야 할 한계가 있다. 국민건강영양조사는 병원이나 검진기관에 실제로 들어오는 사람들의 흐름, 즉 현장 데이터가 아니다. 매년 전국에서 표본을 추출해서 진행하는 단면조사(cross-sectional survey)다. '뇌졸중' 변수도 영상 판독이나 진료 기록이 아니라 "의사에게 뇌졸중을 진단받은 적이 있습니까"라는 설문 문항에 대한 자가보고다. 그리고 조사 시점에 이미 진단을 받은 사람과 아닌 사람을 나눈 것이지, 앞으로 몇 년 안에 뇌졸중이 발생할 사람을 전향적으로 추적한 게 아니다.
...
숫자만 보면 꽤 쓸만한 모델처럼 보인다. 그런데 이 AUC 뒤에 숨어있는 문제가 있다. threshold를 기본값 0.5로 두고 실제로 예측을 시켜보면, 로지스틱·랜덤포레스트·그래디언트부스팅 세 모델은 뇌졸중 사례를 단 한 건도 양성으로 잡아내지 못한다. recall이 정확히 0이다. 이 문제는 2부에서 다룬다.
